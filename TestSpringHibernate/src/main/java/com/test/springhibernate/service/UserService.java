...
import com.test.springhibernate.entity.User;
...
public class UserService {
    ...
    public User signin(String email, String password) {
        ...
        List<User> users = (List<User>) hibernateTemplate.find("FROM User WHERE email=? AND password=?", email, password);
        return users.isEmpty() ? null : users.get(0);
    }

    public User login(String email, String password) {
        @SuppressWarnings({"deprecation", "unchecked"})
        List<User> users = (List<User>) hibernateTemplate.findByNamedQuery("login", email, password);
        return users.isEmpty() ? null : users.get(0);
    }
